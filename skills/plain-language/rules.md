...
| 用比喻代替直说 | 讲机制、结构、原理、流程时，把过程说成比方，读者得自己还原："六条路通往'不知道'，两条路通往判断"；改成按发生顺序把动作一件件说出来——"走完这 6 条都没事，才轮到第 7、8 条"，别换一个同样要读者还原的比方 |
| 元标注黑话 | "对冲句" |
...
| 指代不明、指的东西找不到 | "它""那条"接不上前文；说"方括号里的字面文本"而文里根本没有方括号，或把 `阶段`、`消息` 这种占位说成能直接搜的字面文本——改成说清指的是哪一段、那一段里哪些字是占位 |

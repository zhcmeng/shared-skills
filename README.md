...
1. **你最后在哪个会话敲过字**——标记文件记着。这是每个会话自己的属性，不用调窗口接口。
2. 第 1 条说不是你时，再**看一眼前台窗口的标题**——标题里带着会话名（终端标签上显示的就是
   它）。因为第 1 条记的是「你最后在哪儿敲过字」，不是「你现在看着哪儿」：两个窗口轮流用
   的时候这两件事会分家，你在 A 敲完就切到 B，A 于是成了「后台」，它一答完就弹，可你正
   看着它。

这里原先写着「判定不看窗口焦点」，理由是多个会话常常共用一个终端窗口、看窗口分不出你在
哪个标签。这个理由不成立——窗口标题跟着当前标签走，恰恰分得出，第 2 条就是为此加的。

问不到窗口标题（别的终端程序、拿不到前台窗口），或者这个会话还没有名字，就只看第 1 条。
问不到不能当成「你没在看」——那会让该提醒的时候不提醒。

...
通知是**常驻**的：不点掉就一直留在屏幕上，不会几秒后自己消失。同一个会话再出事，新通知顶
掉旧的，不会堆成一串；你切回某个会话敲字，它的通知会自动撤掉——不撤的话，你答完那道选择题，
「等你选」还挂在屏幕上。

几个会话同时有事时，屏幕上一次只显示一条，点掉一条下一条才顶上来（通知中心里是都在的，
实测过）。这是系统对常驻通知的排队方式，不是我们控制的。

标题用会话标题，也就是终端标签上显示的那个名字，所以看一眼标签就知道该切哪个窗口。

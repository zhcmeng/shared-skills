...
```
claude plugin eval evals/plain-language
```
...
```
claude plugin eval evals/plain-language --case 09-edit-file --scaffold --allow-tools Edit Write
```
...
正例（01～06、09～12）里的 `skill-fired.md` 就是这个指示灯，故意不计分；反例（07、08）里的 `skill-not-fired.md` 写了 `arm: both`，计分——"这个 skill 不该被调用"是这两条用例要断言的。
...
### body-only（9 份，整份文件逐字相同）

01、02、03、04、05、06、11、12、13 各有一份 `graders/body-only.md`，内容完全一样。

## 别被 results/ 里的旧记录带偏

`results/` 下是以前跑出来的记录，那会儿用例还直接放在 `evals/` 底下，路径写的是 `evals/01-jargon-identifiers` 这种，跟现在的目录对不上。要重新跑一遍才有能看的记录。

...
下面开头那几行 import 并到文件顶部：`import io` 与 `import tempfile` 放进 `import json` 那一组（按字母序，`io` 在 `json` 前面），`import convert` 放在 `sys.path.insert` 之后、`import aistudio` 旁边（按字母序，`aistudio` 在 `convert` 前面）。

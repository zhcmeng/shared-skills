...
`from unittest import mock`、`import requests`、`import aistudio`、`from aistudio import …` 这几行并到文件顶部，测试类与 `FakeResponse` 插在 `if __name__ == "__main__":` 之前。测试**不发真实请求**，用假的响应对象喂进去：
...
grep -n "]\(http\|<div\|<span\|<center\|<img\|<image\|<table\|第 *[0-9]\+ *页" "$MD" | head
```

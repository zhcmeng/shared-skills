...
class TestConvertOne(unittest.TestCase):
    ...
    def test_writes_md_and_images(self):
        ...
        # 只写一半，134 条**一条不红**——而 md 里那句 ![](images/x.jpg) 照样
...
块顶那三行 `import argparse` / `import io` / `from concurrent.futures import Future`
是要合进文件顶部那个 import 区的（`import io` 在 Task 10 已经加过了，别再重复加；
`import argparse` 按字母序排在 `import aistudio` 那一组之前，`from concurrent.futures`

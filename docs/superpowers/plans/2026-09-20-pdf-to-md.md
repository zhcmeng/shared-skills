...
`from unittest import mock`、`import requests`、`import aistudio`、`from aistudio import …` 这几行并到文件顶部（`import unittest` 那一组的后面、`sys.path.insert` 那行之前放 `from unittest import mock` 与 `import requests`；`import aistudio` 与 `from aistudio import …` 放在 `sys.path.insert` **之后**，按字母序排在 `from markdown import …` 前面）。测试类与 `FakeResponse` 插在 `if __name__ == "__main__":` 之前。测试**不发真实请求**，用假的响应对象喂进去。

**`JobFailed` 这一轮不要 import**——本任务八条用例一条都用不到它，加了就是没用的 import（同 Task 1 的 `import json` 那处处置）。Task 8 的用例开始用它，那时再加进这一行。

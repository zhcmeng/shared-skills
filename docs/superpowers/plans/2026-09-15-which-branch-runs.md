...
**本节记的是 2026-09-15 修复轮之前的状态，缺口当时是真的。** 那时 `hooks/verify.sh` 只覆盖
bash 分支。**批处理分支——Windows 上的生产路径——没有任何自动检查**，
...
**这条裁定此后已被推翻，缺口也已补上（`59b4429`）。** 推翻它的依据是实测：健康仓库上两条路径的
输出逐字节相同（2449 字节、`cmp` 无差异），这个检查不会误报。现在 `verify.sh` 会在 Windows 上
真跑一遍 `cmd.exe`、比对批处理分支的输出，坏了就报 FAIL。README「维护」节里那条人工步骤
降级成排查手段，不再是覆盖它的唯一手段。

...
> 2026-09-19 追记：限定语此后加了下半句点明时序——「你写给人看的文字，一律按下面这份要求来，
> **落笔时就按它写，不是写完再拿它检查**：回答、文档、说明、提交信息正文。」上面那份逐字记录
> 只对 2026-09-15 当时成立。改的理由：`rules.md` 的表述从「错在哪、怎么改」翻成了落笔时的要求
> （表里左边「写的时候」、右边「别写成这样」），限定语要跟着说明这份东西在什么时候起作用。
> 这半句是往每个会话的上下文里加字，取舍见该次提交的正文。
